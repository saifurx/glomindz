...
class Orders_Model extends Model {
	...
	public function add_cart(){
		...
		$query="SELECT price.id as price_id,price.unit_price as unit_price,package.quantity_in_box as quantity_in_box,customer.customer_type,product.tariff_sub_heading FROM $tables $where";
		//echo $query;
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		...
		if($count == 1){
			...
			$quantity_in_box=$result[0]['quantity_in_box'];
            $tariff_sub_heading=$result[0]['tariff_sub_heading'];
			...
			if($return>0){
                ...
                if($tariff_sub_heading=='30045020'){
                	$ced= 0.02;
				}
				
                $data['central_excise_duty'] = round($data['quantity'] * $quantity_in_box * $unit_price * $ced, 2);//$quantity_in_box * $unit_price * $ced;
                $data['education_cess'] = round($data['central_excise_duty'] * $ced, 2);
                $data['higher_education_cess'] = round($data['central_excise_duty'] * ($ced/2), 2);
	...
	public function save_order(){
		...
		try {
            ...
            $data_sales['central_excise_duty']=$_POST['central_excise_duty'];
			$data_sales['education_cess']=$_POST['education_cess'];
			$data_sales['higher_education_cess']=$_POST['higher_education_cess'];
			
			$data_sales['tax_amount']=$_POST['tax_amount'];
			$data_sales['round_off']=$_POST['round_off'];
			$data_sales['bill_amount']=$_POST['bill_amount'];
			$data_sales['cumulative_bill_amount']=$this->get_customer_cumulative_sales_amount($_POST['customer_id'])+floatval($_POST['bill_amount']);
	...
	public function search_order($search_query){
		...
		$where="orders.customer_id = customer.id and orders.status='Open' and customer.company like '$search_query%' or customer.name like '$search_query%' order by orders.last_update";
