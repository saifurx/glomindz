...
class Invoices_Model extends Model {
	...
	public function get_invoices(){
		...
		$where="where invoice.order_id=orders.id and orders.customer_id=customer.id and invoice.status='Open' order by invoice.last_update desc limit 0,50";
		$query="SELECT invoice.id as invoice_id, invoice.last_update, invoice.vehicle_no, invoice.order_id,customer.company,customer.name,customer.mobile,orders.booking_station,invoice.devilery_date,invoice.despatch_through FROM $tables  $where";
		//echo $query;
	...
	public function update_stock_master($data){
		...
		if($count == 1){
			...
			$error ='Multiple row error in stock master';
	...
	function get_ordered_product_list_print_invoice($order_id){
		...
		$where=" where items.order_id=$order_id and price.id=items.price_id and price.package_id=package.id and product.id=price.product_id";
		$query="SELECT product.name, items.quantity, items.central_excise_duty, items.education_cess, items.higher_education_cess, price.id price_id, price.product_id, price.package_id,price.mrp, price.unit_price, package.quantity_in_box,package.volume, package.type, product.tariff_sub_heading  FROM $table_order_item_list items, $table_price_master price, $table_package_master package,$table_product_master product  $where";
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$result = $sth->fetchAll();
		...
		foreach ($result as $row){
			...
			$data['stocks'] =$this->get_stock_detsils_print($product_id,$package_id,$order_id);

			$data['tariff_sub_heading'] = $row['tariff_sub_heading'];
			$data['central_excise_duty'] = $row['central_excise_duty'];
			$data['education_cess'] = $row['education_cess'];
			$data['higher_education_cess'] = $row['higher_education_cess'];
	...
	function  get_stock_detsils_print($product_id,$package_id,$order_id){
		$return_array=array();
		$table_stock_ledger=TABLE_PREFIX."_stock_ledger";
		$query="SELECT id,location,batch_id,batch_no,exp_date,mfg_date,quantity FROM $table_stock_ledger where product_id=$product_id and package_id=$package_id and transaction_type='Sale' and transaction_reference_no=$order_id order by batch_no";
	...
	function check_stock_availibility($data_array){
		...
		$return_array=array();
		$table_stock_master=TABLE_PREFIX."_stock_master";
		$query="SELECT id, available_unit,location,batch_id,batch_no,exp_date,mfg_date FROM $table_stock_master where product_id=$product_id and package_id=$package_id and transaction_type !='Transfer-pending' and location ='$booking_station' order by batch_no";
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$result = $sth->fetchAll();
	...
	public function get_order_products_footer($order_id){
		...
		foreach ($data as $row){
			...
			$result['central_excise_duty']=$row['central_excise_duty'];
			$result['education_cess']=$row['education_cess'];
			$result['higher_education_cess']=$row['higher_education_cess'];
		}
