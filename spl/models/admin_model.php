...
class Admin_Model extends Model {
	...
	function get_discounts($customer_id){
		...
		$query="SELECT discount.id, discount.name, discount.percentage FROM $table_customer customer,$table_discount discount where customer.id=$customer_id and customer.customer_type=discount.customer_type and discount.status=1";
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$data=$sth->fetchAll();
	...
	function get_taxes($customer_id){
		...
		$sth = $this->db->prepare("SELECT * FROM $table where status=1");
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$data=$sth->fetchAll();
		echo json_encode($data);
	...
	function get_customer_outstanding($customer_id){
		...
		$out_standing = $total_payments-$total_sales;
		$result=array();
	...
	public function get_current_stocks(){
		...
		$table_package=TABLE_PREFIX."_package_master";
		$table="$table_stock_master stock, $table_product product, $table_package package";
		$select="SELECT package.id as package_id,stock.id stock_id, product.short_code, product.name,package.type,package.volume,package.quantity_in_box,stock.available_unit, stock.batch_no, stock.exp_date, stock.mfg_date,stock.location,stock.transaction_type	FROM";
		$where="where stock.product_id=product.id and stock.package_id=package.id and stock.transaction_type!='Transfer-pending'";

		$query=$select.' '.$table.' '.$where.' order by stock.location,product.name,package.type desc ';
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$data=$sth->fetchAll();
