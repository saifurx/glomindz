		...
		<div class="span12">
			<div class="form-inline">
				<input class="input-xlarge" type="text"
					placeholder="Search for block" id="typeahead" autocomplete="off"
					data-provide="typeahead">
				<button id="download_pdf" class="btn pull-right" onclick="printPdf()">download & print</button>
			</div>
			<hr>
		</div>
	</div>
		...
		<div class="span8">
			...
			<div class="row">
				...
				<div class="span4">
					...
					<div><strong>A. Suggested Enterprises :</strong></div>
		...
		success: function(data) {
			for (var i in data){
		     		if(data[i].score >= 88){
  		     		...
  		     		if(data[i].score  > 0 && data[i].score < 66){
						if(data[i].type_id ==1){
...
function printPdf(){
		var id = $('#id').val();
   	 	window.open('<?php echo URL;?>profile/block_profile_print/'+id);
}
