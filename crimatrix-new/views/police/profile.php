			...
			<div class="col-md-4">
				<form role="form" id="profile_form">
				  ...
				  <div class="form-group">
				    ...
				    <select class="selectCity form-control" id="city_id" name="city_id"></select>
				  </div>
				  <div class="form-group">
				    <label class="normal_lable">Police Station</label>
				    <select class="selectPs form-control" id="ps_id" name="ps_id"></select>
				  </div>
				  <br>
				   <div class="form-group">
				  	<button type="button" class="btn btn-primary" id="updateProfile_btn" onclick="updateProfile();">Update</button>
				  	<button type="button" class="btn btn-danger pull-right" data-toggle="modal" data-target="#myModal">Change Password</button>
				  </div>
				</form>
			</div>
		</div>
...
window.onload = function() {
	$('.policeProfile_li').addClass('active');
	get_cities();
	get_policeStations();
...
function get_user_profile_details(){
	...
	$.ajax({
		url: '<?php echo URL;?>police_service/get_police_user_profile_details/',
		type: 'POST',
		dataType: 'JSON',
		...
		success: function(resp){
			...
			for(var i in resp){
				...
				$('#mobile').val(resp[i].mobile);
				$('#email').val(resp[i].email);
				$('#designation').val(resp[i].designation);
...
function updateProfile(){
	...
	if(validName == true && validEmail == true && validMobile == true){
		$.ajax({
			...
			success: function(data){
				if(data == 1){
					alert('Successfuly Updated. Now you will be signed out');
					var url = "<?php echo URL;?>user_service/logout";
					window.location.assign(url);
				}
				if(data == 0){
					alert('Connot be Updated. Please Try Later');
				}
			}
		});
	}
