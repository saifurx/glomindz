<style>
.thumbnail {
	margin-bottom: 10px;
}
</style>
<div id="wrap">	
	<div class="container">
		<div class="row">
			<div class="col-md-3"  style="padding-bottom: 10px; background: antiquewhite; padding-top: 10px; margin-top: -20px;">
				Police station
				<select class="selectPs form-control input-sm" id="ps_id"></select>
			</div>
			<div class="col-md-7">
			<div class="input-group">
			<input style="margin-left: 15px;" class="form-control" type="text" placeholder="Search" id="searcharrest">
            <span class="input-group-btn">
              <button class="btn btn-default" type="button" onclick="searchArrest();"><span class="glyphicon glyphicon-search"></span></button>
            </span>
          </div>
				
			</div>
			<div class="col-md-2" style="padding-bottom: 10px; margin-top: -20px;">
				<br>
				<div class="btn-group btn-group-sm pull-right">
				  <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add_arrest_Modal"><span class="glyphicon glyphicon-paperclip"></span> Add</button>
				  <button type="button" class="btn btn-default" onclick="window.print();"><span class="glyphicon glyphicon-print"></span> Print</button>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-3" style="background: antiquewhite;min-height: 600px;">
				<ul class="list-group" id="crime_list_ul">
				</ul>
			</div>
			<div class="col-md-9" id="arrestedCriminals">
			</div>
		</div>	
	</div>
</div>		

<!-- Modal -->

<!-- Modal Edit User -->
<div class="modal fade" id="add_arrest_Modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header cm-modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title">Add Arrest</h4>
      </div>
      <div class="modal-body row">
	      <form role="form" id="add_arrest_form" class="form-horizontal">
	      	<div class="col-md-6">
	      		<div class="form-group">
					<img style="width: 150px; height: 150px; " id="photo" src="data:image/jpeg;base64," alt="">
				</div>
				<div class="form-group">
			    	<input type="file" id="imgfiles" name="imgfiles" accept="image/jpeg" onchange="readURL(this);"/>
			    </div>
			    <label class="normal_lable">Name</label>
		  		<input class="form-control" type="text" name="name">
		  		<label class="normal_lable">Father Name</label>
		  		<input class="form-control" type="text" name="father_name">
		  		<label class="normal_lable">Mobile Numbers</label>
		  		<input class="form-control" type="text" name="mobile_nos">
		  	</div>
		  	<div class="col-md-6">	
		  		<label class="normal_lable">Crime Type</label>
		  		<select class="form-control" name="crime_type_id" id="crime_type_id"></select>
		  		<label class="normal_lable">Case Referance No</label>
		  		<input class="form-control" type="text" name="case_ref_no">
		  		<label class="normal_lable">Police Station</label>
		  		<select class="selectPs form-control" name="ps_id"></select>
		  		<label class="normal_lable">Arested Date</label>
		  		<input class="form-control" type="text" name="arested_date" id="arested_date">
		  		<label class="normal_lable">Remarks</label>
		  		<input class="form-control" type="text" name="remarks">
		  		<label class="normal_lable">Address</label>
		  		<textarea class="form-control" rows="1" cols="" name="address"></textarea>
			</div>
		</form>
      </div>
      <div class="modal-footer cm-modal-footer">
      	<span class="pull-left"></span>
        <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-primary" id="add_arrest_btn" onclick="addArrest();">Add</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->


<!-- Arrest Detail Modal -->
<div class="modal fade" id="detailModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body row" id="detailModalBody">
        
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->

<script type="text/javascript">
$('#arrests_li').addClass('active');
window.onload = function() {
	get_crimeType();
	get_policeStations();
	$("#arested_date").datepicker({dateFormat: 'yy-mm-dd'});
	get_arrested_criminal(0,0);
};

function get_crimeType(){
	$.ajax({
		url: '<?php echo URL;?>user_service/get_crimeType/',
		type: 'POST',
		dataType: 'JSON',
		success: function(data){
				$('#crime_list_ul,#crime_type_id').empty();
			for(var i in data){
				$('#crime_type_id').append('<option value='+data[i].id+'>'+data[i].name+'</option>')
				$('#crime_list_ul').append('<li class="list-group-item" onclick="showArrestedCriminal('+data[i].id+')">'+data[i].name+'</li><span>');
			}
		}		
	});
}

function showArrestedCriminal(id){
	var ps_id = $('#ps_id').val();
	get_arrested_criminal(id,ps_id);
}

$("#crime_list_ul").on("click", "li", function() {
	$(this).addClass("active").siblings().removeClass("active");
});

function get_policeStations(){
	$.ajax({
		url: '<?php echo URL;?>user_service/get_policeStations/',
		type: 'GET',
		dataType: 'JSON',
		success: function(data){
			for(var i in data){
				$('.selectPs').append('<option value='+data[i].id+'>'+data[i].name+'</option>')
			}
		}		
	});
}

var data_url = 0;
function readURL(input){
	if(input.files[0].size <= 1048576){
	    if (input.files && input.files[0]){
	        var reader = new FileReader();
	        reader.onload = function (e) {
	            $('#photo').attr('src', e.target.result).width(250).height();
	            var a = $('#photo').attr('src');
	            data_url = a;
	        };
	        reader.readAsDataURL(input.files[0]);
	    }
	}
	else{
		alert('File is too large');
	}
}

function addArrest(){
	var formData = $('form#add_arrest_form').serializeArray();
	formData.push({name: 'data_url', value: data_url});
	var validName = alphanumericValidate(formData[0].value);
	$('#add_arrest_btn').attr('disable', true).html('<i class="fa fa-spinner fa-spin"></i>');
	if(validName == true){
		$.ajax({
			type: 'POST',
			url: '<?php echo URL;?>crime_service/add_arrested_criminal/',
			dataType: 'JSON',
			data: formData,
			success: function(resp){
				console.log(resp);
				$('form#add_arrest_form').each(function(){this.reset();});
				$('#add_arrest_btn').attr('disable', false).html('Add');
				$('#add_arrest_Modal').modal('hide');
				get_arrested_criminal(0);
			}
		});
	}
}

function get_arrested_criminal(crime_type,ps_id){
	$('#arrestedCriminals').html('<p style="text-align: center;margin-top: 20px;"><i class="fa fa-spinner fa-spin fa-2x"></i></p>');
	$.ajax({
		type: 'POST',
		url: '<?php echo URL; ?>crime_service/get_arrested_criminal/',
		dataType: 'JSON',
		data: {crime_type:crime_type,ps_id:ps_id},
		success: function(resp){
			$('#arrestedCriminals').empty();
			if(resp.length == 0){
				$('#arrestedCriminals').html('<div class="col-sm-12 col-md-6 "><h3 class="text-danger">No Record Found</h3></div>');
			}
			else{
				for(var i in resp){
				var content = '<div class="col-sm-4 col-md-3">'
			          +'<div class="thumbnail">'
			        	+'<img title="Click for Detail Information" onclick="openModal('+resp[i].id+');" alt="" style="width: 300px; height: 200px; cursor: pointer;" src="'+resp[i].image_url+'">'
			        	+'<div class="caption">'
			        	+'<lable><strong>'+resp[i].name+'</strong></lable><br>'
			        	+'<span class="normal_lable">'+resp[i].father_name+'</span><br>'
			        	+'<span class="normal_lable">'+resp[i].mobile_nos+'</span><br>'
			        	+'</div></div></div>';
		        	$('#arrestedCriminals').append(content);
				}
			}
		}
	});
}

function searchArrest(){
	var search = $('#searcharrest').val();
	var validSearch = alphanumericValidate(search);
	if(validSearch == true){
		$('#arrestedCriminals').html('<p style="text-align: center;margin-top: 20px;"><i class="fa fa-spinner fa-spin fa-2x"></i></p>');
		$.ajax({
			type: 'POST',
			url: '<?php echo URL;?>crime_service/search_arrested_criminal/',
			dataType: 'JSON',
			data: {search:search},
			success: function(resp){
				$('#arrestedCriminals').empty();
				if(resp.length == 0){
					$('#arrestedCriminals').html('<div class="col-sm-12 col-md-6 "><h3 class="text-danger">No Record Found</h3></div>');
				}
				else{
					for(var i in resp){
					var content = '<div class="col-sm-4 col-md-3">'
				          +'<div class="thumbnail">'
				        	+'<img onclick="openModal('+resp[i].id+');" alt="" style="width: 300px; height: 200px; cursor: pointer;" src="'+resp[i].image_url+'">'
				        	+'<div class="caption">'
				        	+'<lable><strong>'+resp[i].name+'</strong></lable><br>'
				        	+'<span class="normal_lable">'+resp[i].father_name+'</span><br>'
				        	+'<span class="normal_lable">'+resp[i].mobile_nos+'</span><br>'
				        	+'</div></div></div>';
			        	$('#arrestedCriminals').append(content);
					}
				}
			}
		});
	}
}

$("#searcharrest" ).keydown(function() {
	if ( event.which == 13 ) {
		searchArrest();
	}
});

function openModal(id){
	$.ajax({
		type: 'POST',
		url: '<?php echo URL;?>crime_service/arrested_criminal_details/',
		dataType: 'JSON',
		data: {id:id},
		success: function(resp){
			for(var i in resp){
			var details ='<div class="col-md-12"><img src="'+resp[i].image_url+'" alt="" style="width: 100%; max-height: 350px;" class="img-rounded"></div>'
				+'<div class="col-md-12"><br>'
				+'<table class="table table-condensed">'
				+'<tr><td>Name:</td><td>'+resp[i].name+'</td></tr>'
				+'<tr><td>Father:</td><td>'+resp[i].father_name+'</td></tr>'
				+'<tr><td>Mobile:</td><td>'+resp[i].mobile_nos+'</td></tr>'
				+'<tr><td>Crime Type:</td><td>'+resp[i].crime_type+'</td></tr>'
				+'<tr><td>Arrest Date:</td><td>'+resp[i].arested_date+'</td></tr>'
				+'<tr><td>Case Ref No:</td><td>'+resp[i].case_ref_no+'</td></tr>'
				+'<tr><td>Police Station:</td><td>'+resp[i].police_station+'</td></tr>'
				+'<tr><td>Remarks:</td><td>'+resp[i].remarks+'</td></tr>'
				+'<tr><td>Address:</td><td>'+resp[i].address+'</td></tr>'
				+'</table>'
				+'</div>';
			$('#detailModalBody').html(details);
			$('#detailModal').modal('show');
			}
		}
	});
}
</script>