			...
			<form role="form" id="edit_hotel_user_form" class="form-horizontal">
				<div class="col-md-3 col-md-offset-2">
					...
					<div class="form-group">
					    ...
					    <input id="editName" class="form-control input-sm" type="text" name="name" required="required">
					 	<label class="normal_lable">Contact Person</label>
			  			<input id="editContactPerson" class="form-control input-sm" type="text" name="contact_person">
					 	<label class="normal_lable">Email <span id="edit_requiredEmailspan" class="errorText">* required valid input</span></label>
				  		<input id="editEmail" class="form-control input-sm" type="email" name="email" required="required">
				  		<label class="normal_lable">Mobile <span id="edit_requiredMobilespan" class="errorText">* required valid input</span></label>
				  		<input id="editMobile" class="form-control input-sm" type="text" name="mobile" required="required" maxlength="10">
				    </div>
				</div>
				<div class="col-md-4">
	    			<label class="normal_lable">City</label>
		  			<select id="editCity" class="selectCity form-control input-sm" name="city_id"></select>
		  			<label class="normal_lable">Police Station</label>
		  			<select id="editPs" class="selectPs form-control input-sm" name="ps_id"></select>
			  
		  			<label class="normal_lable">No of Rooms</label>
		  			<input id="editNoOfRooms" class="form-control input-sm" type="number" name="no_of_rooms">
		  			<label class="normal_lable">Licence No</label>
		  			<input id="editLicenceNo" class="form-control input-sm" type="text" name="licence_no">
		  			<label class="normal_lable">Locality</label>
		  			<input id="editLocality" class="form-control input-sm" type="text" name="locality">
		  			<label class="normal_lable">Pin</label>
		  			<input id="editPin" class="form-control input-sm" type="text" name="pin">
		  			<label class="normal_lable">Address</label>
		  			<textarea id="editAddress"  rows="3" cols="" class="form-control input-sm" name="address"></textarea>
		  			<br>
					<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModal">Change Password</button>
					<button type="button" class="btn btn-primary pull-right" id="updateProfile_btn" onclick="updateProfile();">Save Changes</button>
				</div>
			</form>
		</div>
	</div>	
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form role="form">
          <div class="form-group">
		  	<p><span class="label label-warning">Note:</span> Your password has to be more than 5 character</p>
		  </div>
		 <div class="form-group">
		    <label class="normal_lable" for="exampleInputPassword1">Password</label>
		    <input type="password" class="form-control" id="exampleInputPassword1" placeholder="Password">
		  </div>
		  <div class="form-group">
		    <label class="normal_lable" for="exampleInputPassword2">Confirm Password</label>
...
window.onload = function() {
	$('#hotelProfile_li').addClass('active');
	get_cities();
	get_policeStations();
	setTimeout(function(){
...
function get_user_profile_details(){
	...
	$.ajax({
		url: '<?php echo URL;?>hotel_service/get_hotel_user_profile_details/',
		type: 'POST',
		dataType: 'JSON',
...
function updateProfile(){
	...
	if(validName == true && validEmail == true && validMobile == true){
		...
		$('#updateProfile_btn').attr('disabled',true).html('<i class="fa fa-refresh fa-spin"></i>');
		$.ajax({
			url: '<?php echo URL; ?>hotel_service/update_hotel_user_profile/',
			type: 'POST',
			dataType: 'JSON',
			data: formData,
			success: function(data){
				if(data == 1){
					...
					$('#updateProfile_btn').attr('disabled',false).html('Save changes');
				}
				else{
...
function updatePassword(){
	...
	if( pass1.length < 5 || pass1.length < 5  || pass1 != pass2){
		$('#result').removeClass('alert-success').addClass('alert-danger').html('<h3>Password Didn&#39;t Matched</h3>');
	}
	else{
		$.ajax({
			url: '<?php echo URL;?>user_service/changePassword/',
			type: 'POST',
			dataType: 'JSON',
			data: {password: pass1},
			success: function(data){
				...
				$('#result').removeClass('alert-danger').addClass('alert-success').html('<h3>Successfully Changed</h3>');
				setTimeout(function(){
					$('result').hide();
					$('#myModal').modal('hide');
			    }, 2000);
