...
class Crime_Service_Model extends Model{
	...
	function get_arrested_criminal(){
		...
		if($crime_type == 0){
			...
			if($ps_id == 0 || $ps_id == 9999){
				$query = "SELECT ac.*, cm.name as crime_type FROM ".TABLE_ARRESRTED_CRIMINAL." ac ,".TABLE_CRIME_TYPE_MASTER." cm WHERE ac.crime_type_id = cm.id AND crime_type_id = $crime_type ORDER BY ac.last_update";
			}
			else{
				$query = "SELECT ac.*, cm.name as crime_type FROM ".TABLE_ARRESRTED_CRIMINAL." ac ,".TABLE_CRIME_TYPE_MASTER." cm WHERE ac.crime_type_id = cm.id AND crime_type_id = $crime_type AND ac.ps_id = $ps_id ORDER BY ac.last_update";
			}
		}
	...
	function search_arrested_criminal(){
		$search = $_POST['search'];
		$query = "SELECT ac.*, cm.name as crime_type FROM ".TABLE_ARRESRTED_CRIMINAL." ac ,".TABLE_CRIME_TYPE_MASTER." cm WHERE ac.crime_type_id = cm.id AND ac.name LIKE  '%$search%' ORDER BY ac.last_update";
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$data=$sth->fetchAll();
	...
	function arrested_criminal_details(){
		$id = $_POST['id'];
		$query = "SELECT ac.*, cm.name as crime_type FROM ".TABLE_ARRESRTED_CRIMINAL." ac ,".TABLE_CRIME_TYPE_MASTER." cm WHERE ac.crime_type_id = cm.id AND ac.id = $id ORDER BY ac.last_update";
		$sth = $this->db->prepare($query);
		$sth->setFetchMode(PDO::FETCH_ASSOC);
		$sth->execute();
		$data=$sth->fetchAll();
